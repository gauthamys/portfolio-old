function Polaroid({imgSrc, description, rotate, date, month, year}) {
    return (
        <div className={`w-48 h-64 bg-white border-2 relative ${rotate ? "rotate-[10deg] origin-top-right" : "rotate-[-10deg] origin-top-right"}`}>
...
function Calendar({rotate, month, date}) {
    return (
        <div className={`absolute flex flex-col w-24 h-24 bg-white rounded-lg ${rotate ? "rotate-[-10deg] left-[-3rem]" : "rotate-[10deg] right-[-3rem]"}`}>
            <div className="w-full h-[40%] rounded-t-lg bg-red-700 text-white text-center font-sans font-bold text-sm p-2">{month}</div>
